...
def show(df_daily, df_fix_cost, sku_map, sku_list, sku_type_map, category_options=None):
    ...
    with st.container():
        ...
        c_cat, c_tag, c_sku, c_clear, c_run = st.columns([1.3, 1.3, 3, 0.5, 1])
        with c_cat:
            sel_category_g = st.selectbox("หมวดหมู่สินค้า", CATEGORY_OPTIONS, key="g_cat")
        with c_tag:
            sel_tag_g = st.selectbox("🏷️ แท็กสินค้า", TAG_OPTIONS, key="g_tag")
    ...
    final_skus_g = filter_skus_by_tag(filter_skus_by_category(pre_final_skus_g, sel_category_g), sel_tag_g)

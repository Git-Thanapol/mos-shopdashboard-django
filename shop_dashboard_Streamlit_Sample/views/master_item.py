    ...
    if ws:
        try:
            ...
            if search_term:
                ...
                mask = pd.Series([False] * len(df_editor_view), index=df_editor_view.index)
                if 'SKU' in df_editor_view.columns:
                    mask |= df_editor_view['SKU'].astype(str).str.lower().str.contains(term, na=False)
                if 'ชื่อสินค้า' in df_editor_view.columns:
                    mask |= df_editor_view['ชื่อสินค้า'].astype(str).str.lower().str.contains(term, na=False)
                df_editor_view = df_editor_view[mask]
                st.caption(f"แสดง {len(df_editor_view)} รายการที่ตรงกับ \"{search_term}\"")

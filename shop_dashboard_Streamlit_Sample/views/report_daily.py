...
def show(df_daily, df_fix_cost, sku_map, sku_list, sku_type_map, category_options=None):
    ...
    try:
        # รายชื่อ tag ทั้งหมดจากตาราง tags โดยตรง (ไม่ใช่เฉพาะที่ถูก assign แล้ว)
        all_tag_names = sorted(t["name"] for t in tag_svc.get_flat_tags())
        TAG_OPTIONS = ["แสดงทั้งหมด"] + all_tag_names
        sku_tag_map = tag_svc.get_sku_tag_map()   # {str_sku: [tag_name, ...]}
    except Exception as _tag_err:
        sku_tag_map = {}
        TAG_OPTIONS = ["แสดงทั้งหมด"]
        st.sidebar.warning(f"⚠️ Tag load error: {_tag_err}")
    ...
    def filter_skus_by_tag(current_skus, selected_tag):
        if selected_tag == "แสดงทั้งหมด": return current_skus
        return [s for s in current_skus if selected_tag in sku_tag_map.get(str(s), [])]
    ...
    with st.container():
        ...
        c_type, c_cat, c_tag, c_sku, c_clear, c_run = st.columns([1.5, 1.2, 1.2, 2.5, 0.5, 1])
        with c_type:
        ...
        with c_tag:
            sel_tag_d = st.selectbox("🏷️ แท็กสินค้า", TAG_OPTIONS, key="d_tag")
        with c_sku: 
    ...
    final_skus_d = filter_skus_by_tag(filter_skus_by_category(pre_final_skus_d, sel_category_d), sel_tag_d)
